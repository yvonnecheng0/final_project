...
class TestUsers(unittest.TestCase):
    mock_users = {"Rodya1866": "38JEbnjw@1H!!", "Sonya4U": "4Jewo0323w9eq", "DunyaIsSad": "0k2oL@j4skjw",
                  "PorfiryLives": "heijoJO@092e"}
    ...
    # Tests that resting the database deletes the tables within too
...
class TestJobs(unittest.TestCase):
    ...
    def testAddJob(self):
        ...
        self.assertEqual(is_job("Dunder Mifflin Paper Company", "Assistant to the Regional Manager", "Summer", TEST_DB),
                         True)
        reset_all(TEST_DB)
    ...
    def testAddMultipleJobs(self):
        ...
        for listing in listings:
            l_dict = format_listing(listing)
            quick_add_job(l_dict, TEST_DB)

        self.assertEqual(is_job_id(3, TEST_DB), True)
        reset_all(TEST_DB)
...
class TestApplications(unittest.TestCase):
    ...
    def setUp2(self):
        ...
        for listing in listings:
            l_dict = format_listing(listing)
            quick_add_job(l_dict, db=TEST_DB)
        for user in self.mock_users.keys():
            register_user(user, user + "@fake.edu", self.mock_users[user], db=TEST_DB)
    ...
    def testAddApp(self):
        ...
        self.assertEqual(is_application(1, 1, db=TEST_DB), True)
        reset_all(TEST_DB)
    ...
    def testNoJob(self):
        self.setUp2()
        x = add_application(1, "17eolivia80-04a0-4148-8756-07jjdjww23f", "2024-07-19 15:51:34 UTC", "Ready to Apply",
                            "2024-07-19 18:30:34 UTC",
                            "Cardi", "t09skd@hotmail.com", db=TEST_DB)
